...
class Nextion : Driver
    ...
    def write_block()
        ...
        if (self.flash_written==self.flash_size)
            log("FLH: Flashing complete - Time elapsed: "+str((tasmota.millis()-self.flash_start_millis)/1000))
            self.flash_mode = 0
            self.ser.deinit()
    ...
    def init(switch_recmod)
        log("NXP: Initializing Driver")
        self.switch_recmod = switch_recmod ? true : false
        log("NXP: switch recmod: " + str(self.switch_recmod))
        self.ser = serial(17, 16, 115200, serial.SERIAL_8N1)
        self.flash_mode = 0
